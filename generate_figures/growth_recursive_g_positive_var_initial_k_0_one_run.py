...
output_dir = "./figures"
plot_name = "growth_recursive_g_positive_var_initial_k_0_one_run"
output_path = output_dir + "/" + plot_name + ".pdf"
tag = "growth_recursive_g_positive_var_initial_k_0_one_run"
api = wandb.Api()
...
for id in results['id'].unique(): 
    ...
    result_g_pos["k_t_sol"] = results[results['id'] == id]["k_t_sol"]
    result_g_pos["c_t_sol"] = results[results['id'] == id]["c_t_sol"]
    result_g_pos["z_t"] = results[results['id'] == id]["z_t"]
    k_t = results[results['id'] == id]["k_t_approx"]
    z_t = results[results['id'] == id]["z_t"]
    id_results[id] = result_g_pos
    ids.append(id)
...
ylim_max = 5.0 #1.1 * np.amax(np.maximum(k_max, quant_result_g0["k_t_sol"]))
...
ylim_min = 0.4 #0.7 * np.amin(np.minimum(id_results[id]["c_t_approx"], id_results[id]["c_t_sol"]))
ylim_max = 3 #1.1 * np.amax(np.maximum(id_results[id]["c_t_approx"], id_results[id]["c_t_sol"]))
plt.ylim([ylim_min, ylim_max])

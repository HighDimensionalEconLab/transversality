...
output_dir = "./figures"
plot_name = "growth_recursive_g0_euler_residuals_grid_reg"
...
tag = "growth_recursive_g0_using_c_grid_ensemble_ADAM"
...
# NN(c) artifact
tag = "growth_recursive_g0_using_c_reg_grid_ensemble_ADAM"
results = get_results_by_tag(api, project, tag, get_test_results=True)
assert results.id.nunique() == 100
results["res_t"] = np.power(results["res_t"], 2.0)
quant_result_g0_reg_c = (
    results.groupby("k_t")[["res_t", "k_t", "k_tp1"]]
    .quantile(list(quantiles.values()))
    .unstack(level=-1)
    .reset_index()
...
plt.title(r"Euler residual$s^2$: $NN = c(k)$, without regularization")
...
plt.plot(
    quant_result_g0_reg_c["k_t"][quantiles["mid_quant"]],
    quant_result_g0_reg_c["res_t"][quantiles["mid_quant"]],
    "black",
)
plt.fill_between(
    quant_result_g0_reg_c["k_t"][quantiles["mid_quant"]],
    quant_result_g0_reg_c["res_t"][quantiles["low_quant"]],
    quant_result_g0_reg_c["res_t"][quantiles["high_quant"]],
    facecolor="black",
    alpha=0.15,
)
...
# plt.legend(by_label.values(), by_label.keys(), prop={"size": params["font.size"]})
# plt.legend(loc="upper left")
plt.title(r"Euler residual$s^2$: $NN = c(k)$, with regularization")
plt.xlabel(r"capital($k$)")
...
ylim_min = 0.8 * np.amin(
    np.minimum(
        quant_result_g0_c["k_tp1"][quantiles["mid_quant"]],
        quant_result_g0_reg_c["k_tp1"][quantiles["mid_quant"]],
    )
)
...
plt.title(r"$k'(k): NN = c(k)$, without regularization")
plt.xlabel(r"capital($k$)")
...
plt.fill_between(
    quant_result_g0_reg_c["k_t"][quantiles["mid_quant"]],
    quant_result_g0_reg_c["k_tp1"][quantiles["low_quant"]],
    quant_result_g0_reg_c["k_tp1"][quantiles["high_quant"]],
    facecolor="black",
    alpha=0.15,
)
ylim_min = 0.8 * np.amin(
...
plt.title(r"$k'(k): NN = c(k)$, with regularization")
plt.xlabel(r"capital($k$)")

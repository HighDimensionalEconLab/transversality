...
results = results[results["t"] < 20]  # subsetted so only first 20 not all 50
parameters = get_results_by_tag(api, project, tag, get_config=True)
max_T_test = (
    20  # hard coded because runs went to 50 by accident int(parameters["test_T"][0]))
)
...
time_window = [4, 7]
ave_value = 0.5 * (
    quant_result_g0["k_t_approx"][quantiles["mid_quant"]][time_window[0]]
    + quant_result_g0["k_t_approx"][quantiles["mid_quant"]][time_window[1]]
)
...
time_window = [4, 7]
ave_value = 0.5 * (
    quant_result_g0["c_t_approx"][quantiles["mid_quant"]][time_window[0]]
    + quant_result_g0["c_t_approx"][quantiles["mid_quant"]][time_window[1]]
)
window_width = 0.0275 * ave_value
matplotlib.rcParams.update({"ytick.labelsize": 8})
